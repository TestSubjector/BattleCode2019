// Transcrypt'ed from Python, 2019-01-11 06:09:13
...
// Transcrypt'ed from Python, 2019-01-11 06:09:14
var pi = Math.PI;
var e = Math.E;
var exp = Math.exp;
...
var astar_search = function (robot, pos_initial, pos_final) {
	...
	var dirs = [tuple ([-(1), 1]), tuple ([1, 1]), tuple ([1, -(1)]), tuple ([-(1), -(1)]), tuple ([0, 1]), tuple ([0, -(1)]), tuple ([1, 0]), tuple ([-(1), 0])];
	var nodes = [null];
	...
	if (utility$2.is_out_of_bounds (occupied_map, pos_final [0], pos_final [1]) || !(passable_map [pos_final [1]] [pos_final [0]])) {
		return tuple ([]);
	}
	...
	var neighbours = function (pos_intermediate) {
		...
		for (var dirc of dirs) {
			...
			if (!(utility$2.is_cell_occupied (occupied_map, new_pos_x, new_pos_y)) && passable_map [new_pos_y] [new_pos_x]) {
...
var move_to_specified_mine = function (robot, unit_signal) {
	var nearest_mine_list = utility$3.get_relative_mine_positions (robot);
	if (robot.step != 0) {
		unit_signal--;
	}
	if (unit_signal < len (nearest_mine_list)) {
		...
		var unit_signal = __mod__ (unit_signal, len (nearest_mine_list));
		var tile_to_move_to = pathfinding.astar_search (robot, tuple ([robot.me.x, robot.me.y]), nearest_mine_list [unit_signal]);
	}
...
var pilgrim_move = function (robot, unit_signal) {
	...
	var fuel_map = robot.get_fuel_map ();
	var occupied_map = robot.get_visible_robot_map ();
	var directions = utility$3.cells_around ();
	for (var direction of directions) {
		if (!(utility$3.is_cell_occupied (occupied_map, pos_x + direction [1], pos_y + direction [0])) && (karb_map [pos_y + direction [0]] [pos_x + direction [1]] == 1 || fuel_map [pos_y + direction [0]] [pos_x + direction [1]] == 1)) {
			return robot.move (direction [1], direction [0]);
		}
	}
...
var pilgrim_full = function (robot) {
	...
	if (karb_map [pos_y] [pos_x] == 1 || fuel_map [pos_y] [pos_x] == 1) {
		...
		for (var direction of directions) {
			if (!(utility$3.is_cell_occupied (occupied_map, pos_x + direction [1], pos_y + direction [0])) && (karb_map [pos_y + direction [0]] [pos_x + direction [1]] != 1 || fuel_map [pos_y + direction [0]] [pos_x + direction [1]] != 1) && passable_map [pos_y + direction [0]] [pos_x + direction [1]] == 1) {
...
var crusader_move = function (robot) {
	var directions = utility$4.cells_around ();
	for (var direction of directions) {
...
var church_build = function (robot, unit_type) {
	...
	var occupied_map = robot.get_visible_robot_map ();
	var passable_map = robot.get_passable_map ();
	var directions = utility$5.cells_around ();
	for (var direction of directions) {
...
var castle = function (robot) {
	...
	var crusader_count = 0;
	var pilgrim_count = 0;
	var preacher_count = 0;
	var prophet_count = 0;
	...
	if (robot.step < 2) {
		...
		else if (preacher_count * 2 < crusader_count) {
			return castle_build (robot, unit_preacher);
		}
		else if (prophet_count * 3 < crusader_count) {
			return castle_build (robot, unit_prophet);
		}
		else if ((total_fuel + total_karbonite) * 0.55 < pilgrim_count) {
			robot.signal (robot.me.signal + 1, 2);
			return castle_build (robot, unit_pilgrim);
		}
	}
...
var castle_build = function (robot, unit_type) {
	...
	var occupied_map = robot.get_visible_robot_map ();
	var passable_map = robot.get_passable_map ();
	var directions = utility$6.cells_around ();
	for (var direction of directions) {
...
var MyRobot =  __class__ ('MyRobot', [BCAbstractRobot], {
	...
	get turn () {return __get__ (this, function (self) {
		...
		else if (__t__ (__eq__ (unit_type, unit_church))) {
			return (function () {
				var __accu0__ = churches;
				return __call__ (__accu0__.church, __accu0__, self);
			}) ();
		}
		else if (__t__ (__eq__ (unit_type, unit_crusader))) {
			return (function () {
				var __accu0__ = crusaders;
				return __call__ (__accu0__.crusader, __accu0__, self);
			}) ();
		}
		else if (__t__ (__eq__ (unit_type, unit_preacher))) {
			return (function () {
				var __accu0__ = preachers;
				return __call__ (__accu0__.preacher, __accu0__, self);
			}) ();
		}
		else if (__t__ (__eq__ (unit_type, unit_prophet))) {
			return (function () {
